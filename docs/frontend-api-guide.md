...
`chat_created.title` теперь составляет LLM по первому сообщению в форме
«Генерация жилья на 5000 жителей» — раньше это был обрезанный текст запроса.
Показывай как есть, свою обрезку убери.
...
- **Спрос по каждой зоне в работе** (`residential` и/или `business`): **число
  ...
  Число без зоны («2000 жителей») — это спрос **на всю территорию**: он делится
  между зонами пропорционально их площади (зоны, названные явно, сохраняют своё
  значение и вычитаются из общего). О разбивке приходит `status` вида «Общий
  спрос 2000 жителей распределён по зонам пропорционально площади: жилая — 1400,
  многофункциональная — 600.»
...
|---|---|---|
| `chat_created` | `{ chat_id, title }` | Создан новый чат (если `chat_id` не передавался). `title` — короткий заголовок вида «Генерация жилья на 5000 жителей», который LLM составляет по первому сообщению; показывай его в списке чатов как есть |
...
1. Первый запрос без `chat_id` → приходит `chat_created` с новым `chat_id`. Сохрани его.
   Там же `title` — заголовок чата (3–6 слов вида «Генерация жилья на 5000
   жителей», составлен LLM по первому сообщению; если LLM недоступна, это просто
   обрезанный текст запроса). Заголовок задаётся **один раз**, при создании
   чата, и на последующих ходах не меняется.
...
```
event: chat_created
data: {"chat_id": "9f3a…", "title": "Генерация жилья на 5000 жителей"}
...
Поток **всегда** заканчивается терминальным событием: любой сбой приходит как
`error` + `done`, поэтому «поток закрылся, а результата нет» — это баг сервиса,
а не штатное состояние. Возможные `stage` у `error`:

| `stage` | Когда |
|---|---|
| `load_blocks` | Из `blocks_file` нечего генерировать — причина в `code` (см. раздел 4) |
| `param_extraction` | LLM недоступен или ответил мусором — параметры разобрать не удалось. Повторять запрос с теми же данными бессмысленно, пока бэкенд не поднимется |
| `generation` | Упал пайплайн генерации |
| `stream` | Непредвиденная ошибка сервиса; `detail` содержит тип исключения |

У `load_blocks`, `param_extraction` и `stream`, помимо `detail` (технический текст
для логов), приходит `message` — готовая формулировка для пользователя; её и нужно
показывать. У `load_blocks` есть ещё машинный `code`.
...
- каждая фича — `Polygon`/`MultiPolygon` с типом функциональной зоны. **Фронту не
  нужно валидировать атрибуты** (в т. ч. требовать `properties.zone`) — бэкенд сам
  найдёт тип зоны и сам объяснит, если не получилось;
- атрибут ищется так: сначала `zone`, `functional_zone_type_name` (плоская выгрузка
  Простора), `functional_zone_type.name` (сырой Urban API), затем любой текстовый
  атрибут, чьи значения похожи на типы зон. Если правила не справились — атрибут
  выбирает LLM, но только из реально присутствующих в файле атрибутов;
- значения приводятся к типам UrbanDB (таблица ниже). Понимаются имена UrbanDB
  (`residential_lowrise`), русские подписи («Жилая зона», «Зона застройки
  индивидуальными жилыми домами», «Общественно-деловая зона»,
  «Многофункциональная зона» …) и мелкие опечатки (`residental`). Нераспознанные
  значения уходят в LLM, ответ которого жёстко ограничен списком типов UrbanDB
  или `none`. **Кривой или выдуманный тип никогда не попадает в генерацию** —
  такие объекты пропускаются с `warning`;
- генерятся только зоны, которые нормализуются в **residential** или **business**.

Что придёт в поток при загрузке (всё со `stage: "load_blocks"`, кроме `status`):

| Событие | `code` | Когда |
|---|---|---|
| `status` | — | «Тип зоны взят из атрибута «…»»; отдельный `status`, если LLM распознал нестандартные значения («Жлая зна» → residential) |
| `warning` | `zone_values_unrecognized` | Часть значений не распознана — список значений с количеством, объекты пропущены |
| `warning` | `zone_value_missing` | У части объектов атрибут пуст |
| `warning` | `zones_out_of_scope` | Пропущены зоны вне residential/business (с разбивкой по типам) |
| `warning` | `non_polygon_geometry` | Пропущены не-полигоны |
| `error` | `blocks_empty` | В файле нет объектов |
| `error` | `zone_attribute_not_found` | Не найден атрибут с типом зоны; `message` перечисляет атрибуты файла и ожидаемые значения |
| `error` | `no_generated_zones` | Нет жилых/общественно-деловых зон; `message` перечисляет найденные типы с количеством |
| `error` | `no_polygons` | Нужные зоны есть, но не полигонами |
| `error` | `invalid_blocks` | Некорректная геометрия/координаты |

После `error` сразу идёт `done` — генерации не будет.
...
`result`/текстового описания получает дополнительное событие перед `done`:
...
```text
event: facade_job
data: {"job_id":"01K2...","status_url":"https://facades.example.com/jobs/01K2...","facade_style":"Скандинавский"}
```
