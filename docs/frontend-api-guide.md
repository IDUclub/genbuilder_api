...
| `stage` | Когда |
|---|---|
| `load_blocks` | В `blocks_file` нет валидных фич residential/business |
...
У `param_extraction` и `stream`, помимо `detail` (технический текст для логов),
приходит `message` — готовая формулировка для пользователя.
