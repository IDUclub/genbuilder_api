...
class BuildingGenerator:
    """
    Placement of service buildings and associated land plots on a regular cell grid.

    **Input data:**
        - `cells_gdf` (GeoDataFrame of grid cells). Required columns:
            * `geometry` — Polygon of each grid cell
            * `inside_iso_closed` (bool) — membership of the “developable” territory
              (after applying the 3-side rule)
            * `iso_level` (int | float) — isoline ring level (may be NaN outside isolines)
          Optional: `zone_id`, `zone`, `name`, etc.

        - `zones_gdf`: GeoDataFrame of urban blocks (zones) with identifiers
          (auto-detected) that will be joined to cells using a `within` spatial join.

    **Output:**
        A dictionary with the following elements:
        ```
        {
            "cells": GeoDataFrame,            # cells with final flags and assigned services
            "buildings_rects": GeoDataFrame,  # individual building rectangles (residential + service) before merging
            "buildings_merged": GeoDataFrame, # merged polygons by service type
            "service_sites": GeoDataFrame,    # rectangular land plots of services
            "metrics": DataFrame              # uniformity metrics for service site distribution
        }
        ```

    **Logic and rules:**
        - Each service site is a rectangular cluster of cells (`site`) containing
          a building core placed within an inner buffer zone ≥ 1 cell from the edges.
        - Chebyshev gaps (measured in grid cells):
            * distance to nearest residential building ≥ 1 cell (i.e. d ≥ 2)
            * distance between any sites ≥ 1 cell
            * distance between sites of the same service type ≥ 10 cells
        - Round-robin placement by service type: in each iteration,
          one object of each type is placed per zone.
        - Candidate residential cells are fixed before service placement begins.

    **Notes:**
        - All distances, coordinates, and sizes are expressed in grid cells or meters
          according to the CRS of the input grid.
        - The algorithm supports reproducible randomization of service forms and site
          shapes via `service_random_seed`.
        - After generation, residential and service buildings are unified into
    ...
    svc_order: List[str] = field(
        default_factory=lambda: ["school", "kindergarten", "polyclinics"]
    )
    ...
    def fit_transform(
        ...
        if "inside_iso_closed" not in cells.columns:
            raise ValueError(
                "Во входном слое клеток отсутствует колонка 'inside_iso_closed'."
            )
        cells["inside_iso_closed"] = (
            cells["inside_iso_closed"].fillna(False).astype(bool)
        )
        ...
        for i in range(len(cells)):
            if not inside_mask[i]:
                continue
            is_external[i] = (empty_neighs[i] >= self.neigh_empty_thr) or (
                missing[i] > 0
            )
        cells["candidate_building"] = inside_mask & is_external
        cells["candidate_building"] = self._enforce_line_blocks(
            cells, "row_i", "col_j", "candidate_building", self.max_run
        )
        cells["is_building"] = self._enforce_line_blocks(
            cells, "col_j", "row_i", "candidate_building", self.max_run
        )
        ...
        if promote_targets:
            ...
            cells["is_building"] = self._enforce_line_blocks(
                cells, "row_i", "col_j", "is_building", self.max_run
            )
            cells["is_building"] = self._enforce_line_blocks(
                cells, "col_j", "row_i", "is_building", self.max_run
            )

        idx_by_rc = {
            (int(r), int(c)): i
            for i, (r, c) in enumerate(zip(cells["row_i"], cells["col_j"]))
        }
        ...
        zids_series = (
            cells[zid_col].astype("Int64")
            if zid_col in cells.columns
            else pd.Series([None] * len(cells))
        )
        for zid, sub_all in cells[pd.notna(zids_series)].groupby(zids_series):
            ...
            r_center = (
                float(cells.loc[sub_res, "row_i"].median())
                if len(sub_res)
                else float(sub_all["row_i"].median())
            )
            c_center = (
                float(cells.loc[sub_res, "col_j"].median())
                if len(sub_res)
                else float(sub_all["col_j"].median())
            )
            Lmax = (
                int(
                    pd.to_numeric(cells.loc[in_ids, "iso_level"], errors="coerce")
                    .fillna(0)
                    .max()
                )
                if in_ids
        ...
        svc_count_by_zone = {
            int(z): {s: 0 for s in self.svc_order} for z in zone_groups.keys()
        }
        ...
        for zid, meta in zone_groups.items():
            ...
            limits = {
                svc: self.max_services_per_zone.get(svc, 0) for svc in self.svc_order
            }
                ...
                for svc in self.svc_order:
                    ...
                    for L in range(Lmax, -1, -1):
                        ...
                        if self._try_place_site_and_service_in_zone_level(
                            ...
                            c_cen,
                            placed_site_sets,
                            placed_sites_by_type,
                            rng,
                            shape_variants,
                            idx_by_rc,
                            reserved_site_cells,
                            reserved_service_cells,
                            neighbors_all,
                            service_sites_geom,
                            service_sites_attrs,
                            service_polys_geom,
        ...
        for i in range(len(cells)):
            ...
            diag_in = any(
                is_b[j]
                for j in neighbors_all.get(i, [])
                if j in neighbors_diag.get(i, [])
            )
            if (not side_in) and diag_in:
                diag_only_nodes.append(i)

        diag_adj = {
            i: [j for j in neighbors_diag.get(i, []) if is_b[j]]
            for i in range(len(cells))
        }
        ...
        for k, comp in enumerate(diag_components, start=1):
            pts = np.array(
                [[centroids[i].x, centroids[i].y] for i in comp], dtype=float
            )
            if len(pts) < 2:
                continue
            ...
            ordered_pts = ordered_pts[
                np.concatenate(
                    ([True], np.any(np.diff(ordered_pts, axis=0) != 0, axis=1))
                )
            ]
        ...
        for i in simple_ids:
            ...
            zid_mode = (
                ...
                if zid_col in cells.columns and not pd.isna(cells.at[i, zid_col])
                else None
            )
            records_attrs.append(
                {
        ...
        buildings_rects = gpd.GeoDataFrame(
            records_attrs, geometry=records_geom, crs=cells.crs
        ).reset_index(drop=True)

        left = (
            buildings_rects[["geometry"]].reset_index().rename(columns={"index": "i"})
        )
        right = (
        ...
        svc_vals = (
            buildings_rects.get(
                "service", pd.Series(["living_house"] * len(buildings_rects))
            )
            .astype(object)
            .tolist()
        )
        ...
        pairs = pairs[
            pairs.apply(lambda r: _same_group(int(r["i"]), int(r["j"])), axis=1)
        ]
        ...
        for gid, comp in enumerate(groups):
            ...
            if self.merge_fix_eps and self.merge_fix_eps > 0:
                u = unary_union(
                    [self._make_valid(g.buffer(self.merge_fix_eps)) for g in geoms]
                ).buffer(-self.merge_fix_eps)
            else:
            ...
            types = ",".join(
                sorted(set(buildings_rects.loc[comp, "type"].astype(str).tolist()))
            )
            zid_mode = None
            ...
            merged_geoms.append(self._make_valid(u))
            merged_attrs.append(
                {
                    "group_id": int(gid),
                    "n_members": int(len(comp)),
                    "n_cells_sum": int(
                        np.nansum(buildings_rects.loc[comp, "n_cells"].values)
                    )
                    if "n_cells" in buildings_rects
                    else None,
                    "types": types,
        ...
        buildings_merged = buildings_merged[
            buildings_merged.area > self.cell_size_m * self.cell_size_m
        ]

        service_sites_gdf = gpd.GeoDataFrame(
            service_sites_attrs, geometry=service_sites_geom, crs=cells.crs
        ).reset_index(drop=True)
        metrics_rows: List[Dict] = []
        if len(service_sites_gdf) > 0:
            service_sites_gdf[
                "centroid"
            ] = service_sites_gdf.geometry.representative_point()
            for (zid, svc), sub in service_sites_gdf.groupby(
                [zid_col, "service"], dropna=True
            ):
        ...
        cells.loc[cells["is_building"].fillna(False), "service"] = cells.loc[
            cells["is_building"].fillna(False), "service"
        ].fillna("living_house")

        service_buildings = buildings_merged[
            buildings_merged["service"] != "living_house"
        ]
        living_buildings = buildings_merged[
            buildings_merged["service"] == "living_house"
        ]
        ...
        if len(service_buildings) > 0:
            service_buildings["capacity"] = gpd.sjoin(
                service_buildings, service_sites_gdf, how="left"
            )["capacity"]
            floors_mapping = {"school": 3, "kindergarten": 2, "polyclinics": 4}
            service_buildings["floors_count"] = service_buildings.service.map(
                floors_mapping
            )
        ...
        if self.verbose:
            svc_rects = buildings_rects[
                buildings_rects.get("service").isin(self.svc_order)
            ]
            print(
    ...
    def _components(
        self, nodes: List[int], adj: Dict[int, List[int]]
    ) -> List[List[int]]:
        node_set = set(nodes)
        seen, comps = set(), []
        for v in nodes:
    ...
    def _pattern_library(
        self,
    ) -> Dict[str, List[Tuple[str, List[Tuple[int, int]], bool]]]:
    ...
    def _transform_offsets(
        self, offsets: List[Tuple[int, int]], rot_k: int, mirror: bool
    ) -> List[Tuple[int, int]]:
        out = []
    ...
    def _shape_variants(
        self, offsets: List[Tuple[int, int]], allow_rotations: bool
    ...
    def _min_cheb_between_sets(
        self, cells: gpd.GeoDataFrame, A: List[int], B: List[int]
    ) -> int:
        if not A or not B:
            return 10 ** 9
    ...
    def _min_site_cells_for_service_with_margin(
        self,
        svc: str,
        shape_variants: Dict[str, List[Tuple[str, List[List[Tuple[int, int]]]]]],
        inner_margin_cells: int = 1,
    ...
    def _sort_variants_by_core_fit(
        self, variants: List[List[Tuple[int, int]]], core_h: int, core_w: int
    ) -> List[List[Tuple[int, int]]]:
    ...
    def _try_place_service_inside_site(
        ...
        for (pat_name, variants) in shape_variants.get(svc, []):
            ...
            for var in vars_iter:
                ...
                positions.sort(
                    key=lambda rc: (rc[0] - cen_r) ** 2 + (rc[1] - cen_c) ** 2
                )
    ...
    def _cheb_gap_ok_to_sites(
        ...
        for S in placed_site_sets:
            if (
                ...
                < self.gap_between_sites_cheb
            ):
                return False
        for S in placed_sites_by_type.get(svc, []):
            if (
                self._min_cheb_between_sets(cells, candidate_idxs, S)
                < self.same_type_site_gap_cheb
            ):
                return False
        return True
            ...
            for (site_form_name, site_offsets) in territory_variants:
                ...
                for (r0, c0) in positions:
                    ...
                    (
                        ...
                        svc_cell_idxs,
                        chosen_pat,
                    ) = self._try_place_service_inside_site(
                        cells,
                        svc,
                        zid,
                        site_idxs,
                    ...
                    service_id = (
                        f"{svc.upper()}_{str(len(service_polys_attrs) + 1).zfill(4)}"
                    )
                    ...
                    site_poly = self._make_valid(
                        unary_union([cells.geometry[i] for i in site_idxs])
                    )
                    svc_poly = self._make_valid(
                    ...
                    service_sites_attrs.append(
                        {
                            ...
                            "site_area_actual_m2": float(
                                getattr(site_poly, "area", 0.0)
                            ),
    ...
    def _try_place_site_and_service_fallback_outside(
        ...
        for (pat_name, _service_vars) in service_variants:
            ...
            min_cells = self._min_site_cells_for_service_with_margin(
                svc, shape_variants, self.inner_margin_cells
            )
