...
class BuildingGenerator:
    ...
    **Notes:**
    ...
    def fit_transform(
        ...
        service_buildings = buildings_merged[buildings_merged['service'] != 'living_house']
        living_buildings = buildings_merged[buildings_merged['service'] == 'living_house']
        living_buildings['service'] = None
        living_buildings['is_living'] = True

        if len(service_buildings) > 0:   
            service_buildings['capacity'] = gpd.sjoin(service_buildings, service_sites_gdf, how='left')['capacity']
            ...
            service_buildings['service'] = service_buildings.apply(lambda x: [{x['service']: x['capacity']}], axis=1)
