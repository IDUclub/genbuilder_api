...
- **stored layers** (``buildings``, ``blocks_input``, ``existing_buildings``,
  ``zones``) are our own artefacts, written to object storage and served back by
  ``/files/{slot}/{result_id}``. ``zones`` holds the zones taken from an uploaded
  blocks file in the project-less mode, where no UrbanDB scenario exists;
...
_SLOT_SPECS: dict[str, tuple[str, str, str]] = {
    ...
    # Same name and title as the live scenario layer: the map shows one zones
    # layer whichever mode produced it.
    SLOT_ZONES: (ZONES_LAYER_NAME, "Функциональные зоны", "input"),
}
